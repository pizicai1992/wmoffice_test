...
function run_newuser() {
    ...
    insert overwrite table bi_dwb.t_dwb_acct_$1_newuser_d partition
    (par_dt)
    select userid,
            account,
        ...
        from (select userid,
                    ...
                    today_firstlogin_time,
                    today_firstlogin_ip,
                    row_number() over(partition by userid order by today_firstlogin_time) rn
                from bi_dwb.t_dwb_act_$1_user_active_d t
            where par_dt >= '$2-01-01' and par_dt<'$3-01-01'
                and today_login_counts > 0) t
    where t.rn = 1
        and not exists (select 1
                from bi_dwb.t_dwb_acct_$1_newuser_d s
            where s.par_dt < '$2-01-01'
                and t.userid = s.userid);
    "
    hive -v -i /var/lib/impala/test/dongtai.conf -e "${hive_sql}"
    if [ $? -eq 0 ];then
        echo "[`date +"%Y-%m-%d %H:%M:%S"` - INFO] load dwb run_newuser SUCCESS !"
    else
        echo "[`date +"%Y-%m-%d %H:%M:%S"` - ERROR] load dwb run_newuser FAILED !"
    fi
}
...
while [[ $start_year < $stop_year ]]; do
    ...
    echo "run_newuser ${gameid_long} ${start_year} ${end_year}"
    run_newuser ${gameid_long} ${start_year} ${end_year}
    sleep 3
