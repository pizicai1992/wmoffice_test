...
def get_columninfo(mysql_host, username, password, mysql_tab, mysql_db):
...
def print_sqoopcmd(column_str, jdbc, mysql_user, mysql_table, mysql_db, hive_db, hive_table, logpath, file_format='textfile', split_column='', map_num=1, where_condition='', part_key='', part_value=''):
    sqoop_cmd_str = ''
    cmd_dict = {}
...
def main():
    ...
    parser.add_argument('-o', '--mysqltabe', required=True,
    ...
    columns = get_columninfo(mysql_host=mysql_host, username=mysql_user,
                             password=mysql_password, mysql_tab=mysql_table, mysql_db=mysql_base)
