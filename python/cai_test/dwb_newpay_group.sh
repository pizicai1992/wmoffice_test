...
function run_newpay_group() {
    ...
    insert overwrite table bi_dwb.t_dwb_acct_$1_newpayuser_group_d partition
    (par_dt)
    select userid,
            groupname,
            today_firstpay_time,
        ...
        from (select userid,
                    ...
                    today_firstpay_cash,
                    par_dt,
                    row_number() over(partition by userid,groupname order by today_firstpay_time) rn
                from bi_dwb.t_dwb_econ_$1_user_pay_d t
            where par_dt >= '$2-01-01' and par_dt<'$3-01-01') t
    where t.rn = 1
        and not exists (select 1
                from bi_dwb.t_dwb_acct_$1_newpayuser_group_d s
            where s.par_dt < '$2-01-01'
                and s.userid = t.userid
                and s.groupname = t.groupname);
    "
    hive -v -i /var/lib/impala/test/dongtai.conf -e "${hive_sql}"
    if [ $? -eq 0 ];then
        echo "[`date +"%Y-%m-%d %H:%M:%S"` - INFO] load dwb run_newpay_group SUCCESS !"
    else
        echo "[`date +"%Y-%m-%d %H:%M:%S"` - ERROR] load dwb run_newpay_group FAILED !"
    fi
}
...
while [[ $start_year < $stop_year ]]; do
    ...
    echo "run_newpay_group ${gameid_long} ${start_year} ${end_year}"
    run_newpay_group ${gameid_long} ${start_year} ${end_year}
    sleep 3
