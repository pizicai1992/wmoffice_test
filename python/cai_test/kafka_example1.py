# encoding:utf-8 __author__ = 'eric.sun'
"""演示如何使用Spark Streaming 通过Kafka Direct Streaming实现WordCount
执行命令：./spark-submit --master spark://server1-1-5-24-137:7077
--packages org.apache.spark:spark-streaming-kafka_2.10:1.6.0
../examples/kafka_streaming.py > log Kafka数据源程序：
https://github.com/Eric-aihua/practise.git/java_cookbook/cookbook/src/main/java/com/eric/kafka/producer/ProcuderSample.java
使用Direct的好处 1：根据topic的分区数默认的创建对应数量的rdd分区数
2：Receiver的方式需要通过Write AHead Log的方式确保数据不丢失，Direct的方式不需要
3：一次处理：使用Kafka Simple API对数据进行读取，使用checkpoint对offset进行记录
问题： 基于Zookeeper的Kafka监控工具不能获取offset的值了，需要在每次Batch处理后，
可以对Zookeeper的值进行设置 """
...
# Start the computation ssc.awaitTermination() # Wait for the computation to terminate
offsetRanges = []
